...
public class PraatTextFile extends PraatFile {
    ...
    private static jregex.Pattern STRING_PATTERN = new jregex.Pattern("\"({target}.*)\"") ;
    private static jregex.Pattern INTEGER_PATTERN = new jregex.Pattern("(?!\\[)({target}\\d+)(?!\\])");
    private static  jregex.Pattern DOUBLE_PATTERN = new  jregex.Pattern("(?!\\[)({target}\\d+(\\.\\d+)?)(?![\\d\\]])");
